import * as React from 'react';
import { useForm, Controller } from 'react-hook-form';
import Box from '@mui/material/Box';
import Stepper from '@mui/material/Stepper';
import Step from '@mui/material/Step';
import StepLabel from '@mui/material/StepLabel';
import Button from '@mui/material/Button';
import Typography from '@mui/material/Typography';
import { Card, TextField, Checkbox, Select, MenuItem } from "@mui/material";
import {Step1, Step2, Step22} from "./Steps";

type FormData = {
    multiline1: string;
    checkbox1: boolean;
    dropdown1: string;
};

export default function MultiStepper() {
    const steps = ['Insert Document', 'Correct Document', 'Confirm Document', 'Set Analyze Settings'];
    const { control, handleSubmit, formState: { errors } } = useForm<FormData>({
        mode: 'onTouched',
        defaultValues: {
            multiline1: '',
            checkbox1: false,
            dropdown1: '',
        },
    });

    const [activeStep, setActiveStep] = React.useState(0);

    const handleNext = () => {
        handleSubmit((data) => {
            console.log(data); // Perform validation logic and submit data
            setActiveStep((prevActiveStep) => prevActiveStep + 1);
        })();
    };

    const handleBack = () => {
        setActiveStep((prevActiveStep) => prevActiveStep - 1);
    };

    const handleReset = () => {
        setActiveStep(0);
    };

    return (
        <Card sx={{ padding: '2%' }}>
            <Box sx={{ width: '100%' }}>
                <Stepper activeStep={activeStep}>
                    {steps.map((label, index) => {
                        const stepProps: { completed?: boolean } = {};
                        const labelProps: { optional?: React.ReactNode } = {};
                        return (
                            <Step key={label} {...stepProps}>
                                <StepLabel {...labelProps}>{label}</StepLabel>
                            </Step>
                        );
                    })}
                </Stepper>
                {activeStep === steps.length ? (
                    <React.Fragment>
                        <Typography sx={{ mt: 2, mb: 1 }}>
                            All steps completed - you&apos;re finished
                        </Typography>
                        <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
                            <Box sx={{ flex: '1 1 auto' }} />
                            <Button onClick={handleReset}>Reset</Button>
                        </Box>
                    </React.Fragment>
                ) : (
                    <React.Fragment>
                        <Box sx={{ mt: 2, mb: 1 }}>


                            {
                                activeStep === 0 ? (
                                    <Step1 control={control} />
                                ) : activeStep === 1 ? (
                                    <Step2 control={control} />
                                ) : activeStep === 2 ? (
                                    <Step2 control={control} />
                                ) : (
                                    <Step2 control={control} />
                                )
                            }



                        </Box>
                        <Box sx={{ display: 'flex', flexDirection: 'row', pt: 2 }}>
                            <Button
                                color="inherit"
                                disabled={activeStep === 0}
                                onClick={handleBack}
                                sx={{ mr: 1 }}
                            >
                                Back
                            </Button>
                            <Box sx={{ flex: '1 1 auto' }} />
                            <Button onClick={handleNext}>
                                {activeStep === steps.length - 1 ? 'Analyze' : 'Next'}
                            </Button>
                        </Box>
                    </React.Fragment>
                )}
            </Box>
        </Card>
    );
}
